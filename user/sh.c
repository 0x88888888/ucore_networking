...
readline(const char *prompt) {
    ...
    while (1) {
        char c;
        //printf("user readline start\n");
        if ((ret = read(0, &c, sizeof(char))) < 0) {
            //printf("user readline 10\n");
            return NULL;
        }
        else if (ret == 0) {
            //printf("user readline 20\n");
            if (i > 0) {
                buffer[i] = '\0';
                break;
            }
        ...
        //printf("user readline 30\n");

        if (c == 3) {
            return NULL;
        }
        else if (c >= ' ' && i < BUFSIZE - 1) {
            //printf("user readline 40\n");
            putc(c);
            buffer[i ++] = c;
        }
        else if (c == '\b' && i > 0) {
            //printf("user readline 50\n");
            putc(c);
            i --;
        }
        else if (c == '\n' || c == '\r') {
            //printf("user readline 60\n");
            putc(c);
            buffer[i] = '\0';
            break;
        }
...
reopen(int fd2, const char *filename, uint32_t open_flags) {
    int ret, fd1;
    close(fd2);
    if ((ret = open(filename, open_flags, (uint32_t)NULL)) >= 0 && ret != fd2) {
        close(fd2);
...
testfile(const char *name) {
    int ret;
    if ((ret = open(name, O_RDONLY , (uint32_t)NULL)) < 0) {
        return ret;
    }
    close(ret);
    return 0;
