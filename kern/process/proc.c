...
proc_run(struct proc_struct *proc) {
    if (proc != current) {
        ...
        local_intr_save(intr_flag);
        {
            current = proc;
            // 设置Ring0态时候的栈顶
            // 用户空间的堆栈分配在函数load_icode中确定
            load_esp0(next->kstack + KSTACKSIZE); 
            lcr3(next->cr3);//切换 page directory table
...
// init_main-->
//             kernle_thread(user_main,NULL,0)
int
...
// copy_mm - process "proc" duplicate OR share process "current"'s mm accroding clone_flags
...
copy_thread(struct proc_struct *proc, uintptr_t esp, struct trapframe *tf) {
    //iret 返回时
    //设置 trapframe,在switch_to返回的时候用,
    //trapframe的开始地址就是proc在内核态的栈(esp从此开始)的地址
    proc->tf = (struct trapframe *)(proc->kstack + KSTACKSIZE) - 1;
    *(proc->tf) = *tf;
    //fork调用，子线程的返回值为0
    proc->tf->tf_regs.reg_eax = 0;
    proc->tf->tf_esp = esp;//此处的esp为ring3状态的地址
    proc->tf->tf_eflags |= FL_IF;
    
    //线程切换(switch_to)时 ,到函数forkret处。
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    */
    ...
    //复制 current->mm ---> proc->mm
    if (copy_mm(clone_flags, proc) != 0) {
        goto bad_fork_cleanup_fs;
    }

    // 设置子进程的用户态堆栈和子进程的返回值为0
    // 实参stack的值ring3的esp,如果进程stack=0就表示是copy ring0的
    ...
    //将子进程proc放入就绪进程队列排队，将以switch_to式的返回
    wakeup_proc(proc);
 ...
 * @size:  the size of the content of binary 
 *
 * sys_exec-->do_execve-->
 *                        load_icode
 */
...
load_icode(int fd, int argc, char **kargv) {
    ...
    for (phnum = 0; phnum < elf->e_phnum; phnum ++) {
        ...
        // size of segment in file can bigger than size of segment in memory
        ...
        //根据 ph->p_va, ph->memsz 创建一个相适应的vma区域,但是没有映射相应的物理内存
        ...
        // porgram header 在文件中的偏移
        off_t offset = ph->p_offset;
        
        size_t off, size;
        
        // start 表示 program header 对应的section在内存中的位置
    ...
    //cprintf("load_icode brk_start:%x      brk:%x \n",mm->brk_start,mm->brk);
    ...
    // fork之后，重新确定当前process的mm_struct和cr3
    mm_count_inc(mm);
    current->mm = mm;
    ...
    *(int *)stacktop = argc;
    //程序的main参数构建结束，接着需要构建一个trapframe,跳转到用户态去运行

    //(6) setup trapframe for user environment
    // 重新构建 trapframe ，在内核态程序返回的时候，可以执行到相应的代码
    ...
    tf->tf_esp = stacktop;//用户态栈
    tf->tf_eip = elf->e_entry; //ring3代码的入口出
    tf->tf_eflags = FL_IF;
    ret = 0;
out:
    return ret;
...
do_execve(const char *name, int argc, const char **argv) {
    ...
    int fd;
    if ((ret = fd = sysfile_open(path, O_RDONLY, (int32_t)NULL)) < 0) {
        cprintf("open fialed path: %s\n",path);
        goto execve_exit;
    }
    if (mm != NULL) {
        lcr3(boot_cr3);
...
init_main(void *arg) {
    int ret;
    // 设置设别vdev_list中的disk0设备为bootfs_node
    ...
    // init进程中创建另外一个进程，放入就绪队列排队
    int pid = kernel_thread(user_main, NULL, 0);
    if (pid <= 0) {
        panic("create user_main failed.\n");
    }
    
    extern void start_net_mechanics();
    start_net_mechanics();
...
do_sleep(unsigned int time) {
    ...
    return timer->expires > 0 ? timer->expires : 0 ;
}
