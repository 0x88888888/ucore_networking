 ...
 */
int
sysfile_open(const char *__path, uint32_t open_flags, uint32_t arg2) {
	int ret;
	char *path;
	if ((ret = copy_path(&path, __path)) != 0) {
		return ret;
	}
    //cprintf("sysfile_open path: %s\n",__path);
	ret = file_open(path, open_flags ,arg2);
    //cprintf("sysfile_open ret %d\n",ret);
	kfree(path);
	return ret;
}
...
sysfile_write(int fd, void *base, size_t len) {
    ...
    while (len != 0) {
    	...
    	unlock_mm(mm);
    	if (ret == 0) {
            //cprintf("sysfile_write fd=%d buffer=%s \n",fd,buffer);
