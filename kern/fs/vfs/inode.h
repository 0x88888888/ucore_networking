 ...
 *
 *
 * inode连接了 sfs_node和fs的关系
 * 因为包含对sfs_inode或者 device的操作集合，所以代表了inode的抽象
 *
 * inode_ops的函数集合中，各个函数实现都是将inode转到 sfs_inode
 *
 */
struct inode {
...
struct inode_ops {
    ...
    int (*vop_open)(struct inode *node, uint32_t open_flags, uint32_t arg2);
...
#define vop_open(node, open_flags, arg2)                            (__vop_op(node, open)(node, open_flags, arg2))
