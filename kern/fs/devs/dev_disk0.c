...
static int
disk0_open(struct device *dev, uint32_t open_flags, uint32_t arg2) {
	return 0;
}
...
dev_init_disk0(void) {
    ...
    //disk0_device_init(node->in_info->__device__info);
    disk0_device_init(vop_info(node, device));

    int ret;
    //将设备disk0插入都vdev_list中去，在init_main中会用到这个设备
