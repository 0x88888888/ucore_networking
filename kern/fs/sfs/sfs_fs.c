 ...
 * 获取 root block的 inode
 * 
 * vfs_get_root
 *     fsop_get_root(vdev->fs)
 * get_device
 *     fsop_get_root(node->in_fs)
 * 
 */
static struct inode *
 ...
 * fs_init
     sfs_mount
        vfs_mount
           sfs_do_mount
 *
 * 加载位于磁盘上的SFS文件系统的 super block 和 freemap
 *
 * dev =vfs_dev_t->devnode->in_info.__device_info
 */
static int
