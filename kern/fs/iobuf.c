 ...
 * m2b ==true 表示从 data-->iob,否则表示 iob-->data
 ...
 */
void
iobuf_skip(struct iobuf *iob, int32_t n) {
	//cprintf("iob->io_resid =%d \n n=%d\n",iob->io_resid, n);
    assert(((int32_t)iob->io_resid) >= n);
    iob->io_base += n, iob->io_offset += n, iob->io_resid -= n;
}


/*
 * iobuf_at_start - io_base == io_start
 */
bool iobuf_at_start(struct iobuf* buf) {
	assert(buf != NULL);
	return buf->io_base == buf->io_start;
}
