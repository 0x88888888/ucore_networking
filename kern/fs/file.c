 ...
 */
int
file_open(char *path, uint32_t open_flags, uint32_t arg2) {
    ...
    struct inode *node;
    //转到 vfs(抽象层)
    if ((ret = vfs_open(path, open_flags, arg2, &node)) != 0) {
    	fd_array_free(file);
    	return ret;
    }
    ...
    if (open_flags & O_APPEND) {
        ...
        // node->in_op->vop_fstat(node,stat)
...
file_read(int fd, void *base, size_t len, size_t *copied_store) {
	...
	*copied_store = 0;
    ...
    /* 
     * file->node->vop_read(file->node, iob)
     * dev_read
     * sfs_read
     */
...
file_write(int fd, void *base, size_t len, size_t *copied_store) {
    ...
    *copied_store = 0;
    ...
    struct iobuf __iob, *iob = iobuf_init(&__iob, base, len, file->pos);
    //file->node->in_ops->vop_write(file->node, iob);
...
file_seek(int fd, off_t pos, int whence) {
    ...
    if (ret == 0) {
        //file->node->in_ops->vop_tryseek(file->node, iob);
...
file_fstat(int fd, struct stat *stat) {
    ...
    fd_array_acquire(file);
    //file->node->in_ops->file_fstat(file->node, iob);
...
file_fsync(int fd) {
    ...
    fd_array_acquire(file);
    //file->node->in_ops->vop_fsync(file->node, iob);
...
file_getdirentry(int fd, struct dirent *direntp) {
    ...
    /*
     * iob->base =direntp->name
     * iob->io_len = iob->io_resid = sizeof(direntp->name);
     * iob->offset = direntp->offset
     */
    struct iobuf __iob, *iob = iobuf_init(&__iob, direntp->name, sizeof(direntp->name), direntp->offset);
    /*
     * (file->node)->vop_getdirentry(file->node, iob)
     * sfs_getdirentry(file->node, iob)  (文件sfs_inode.c);
     * 
     */
    if ((ret = vop_getdirentry(file->node, iob)) == 0) {
        //成功
        //direntp->offset += iob->io_len - iob->io_resid
