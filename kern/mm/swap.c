	...
	for (i = 0; i != n; ++ i)
	{
		...
		//assert(!PageReserved(page));
          ...
          /*
              2^8 =256
              2^12 = 4096
              page->pra_vaddr/PGSIZE(0x4000)
              相当与 (page->pra_vaddr>>12)/0x4 
              << 8 相当于 只留下 中间的12位表示 
              128M的swap磁盘 可以 保存 32768个物理内存Frame
          ...
          */
          if (swapfs_write( (page->pra_vaddr/PGSIZE+1)<<8, page) != 0) {
                    ...
                    //换出成功，修改page table entry 的值为相应的扇区号
...
check_swap(void)
{
    //backup mem env
     ...
     for (i=0;i<CHECK_VALID_PHY_PAGE_NUM;i++) {
          check_rp[i] = alloc_page();//page对象所在的逻辑地址
