# sudo mount -t vboxsf ucore /home/moocos/share
PROJ	:= 8
EMPTY	:=
SPACE	:= $(EMPTY) $(EMPTY)
SLASH	:= /
...
CC		:= $(GCCPREFIX)gcc
CFLAGS	:= -fno-builtin -Wall  -ggdb -m32 -gstabs -nostdinc $(DEFS) -Wunused-function -Wunused-variable
...
KSRCDIR		+= kern/init \
			   ...
			   kern/net/e1000 \
			   kern/net/tcpip/src \
			   kern/net/tcpip/src/sys \
			   kern/net/tcpip/src/sys/devs \
			   kern/net/tcpip/src/netapp \
			   kern/net/tcpip/src/dhcp \
			   kern/net/tcpip/src/tcp \
			   kern/net/tcpip/src/tcp/tcpsfm \
			   kern/net/tcpip/src/tcp/tcpostate \
			   kern/net/tcpip/src/tcp/tcpprocess \
			   kern/net/tcpip/src/dgram \
			   kern/net/tcpip/src/tcpd \
			   kern/net/tcpip/src/igmp \
			   kern/net/tcpip/src/lib \
			   kern/net/tcpip/src/net  \
			   kern/net/tcpip/src/arp \
			   kern/net/tcpip/src/ip  \
			   kern/net/tcpip/src/rip \
			   kern/net/tcpip/src/snmp \
			   kern/net/tcpip/src/udp \
			   kern/net/tcpip/src/icmp \
			   kern/net/tcpip/src/socket 
...
bochs: $(UCOREIMG) $(SWAPIMG) $(SFSIMG)
	$(V)$(BOCHS) -f sudo bochs -f bochsrc.txt
...
which-ports:
	@echo "Local port $(PORT7) forwards to UCORE port 7 (echo server)"
	@echo "Local port $(PORT80) forwards to UCORE port 80 (web server)"

nc-80:
	nc localhost $(PORT80)

nc-udp-80:
	nc -u localhost $(PORT80)

nc-7:
	nc localhost $(PORT7)

nc-udp-7:
	nc -u localhost $(PORT7)
